...
check_public_story_assets() {
  local path
  for path in \
    "/stories/story-1/cover" \
    "/stories/story-1/pages/1/image" \
    "/stories/story-1/pages/1/audio" \
    "/stories/story-1/outcome/audio"; do
    probe_request GET "$base_url$path"
    record_probe "$path" "$response_status" "$response_time_ms"

    if [[ "$response_status" != "200" ]]; then
      echo "Public story asset check failed for $path with status $response_status" >&2
      exit 1
    fi
...
check_public_story_errors() {
  ...
  if [[ "$response_status" != "400" ]]; then
    echo "Invalid page check failed with status $response_status" >&2
    exit 1
  fi
  mark_check

  probe_request GET "$base_url/stories"
  record_probe "/stories" "$response_status" "$response_time_ms"
  if [[ "$response_status" != "401" ]]; then
    echo "Protected stories list should return 401, got $response_status" >&2
    exit 1
  fi
  mark_check

  probe_request GET "$base_url/stories/story-1/text"
  record_probe "/stories/story-1/text" "$response_status" "$response_time_ms"
  if [[ "$response_status" != "401" ]]; then
    echo "Protected story text should return 401, got $response_status" >&2
    exit 1
  fi
  mark_check

  probe_request POST "$base_url/scan" -F 'file=@/dev/null;type=image/png;filename=empty.png'
  record_probe "/scan" "$response_status" "$response_time_ms"
  if [[ "$response_status" != "401" ]]; then
...
check_token() {
  # Keep API smoke tests non-blocking when credentials are not configured.
  if [[ -z "$username" || -z "$password" ]]; then
    echo "Skipping token/authenticated checks because API_USERNAME/API_PASSWORD were not provided."
    return 1
  fi
  ...
  record_probe "/token" "$response_status" "$response_time_ms"

  if [[ "$response_status" != "200" ]]; then
    echo "Token request failed with status $response_status" >&2
    echo "$response_body" >&2
    exit 1
  fi
...
check_authenticated_story_routes() {
  local token="$1"
...
import json, sys
payload = json.loads(sys.argv[1])
assert payload.get("stories"), payload
PY
  mark_check
  ...
  probe_request POST "$base_url/scan" -H "Authorization: Bearer $token" -F 'file=@/dev/null;type=image/png;filename=empty.png'
  record_probe "/scan(authenticated)" "$response_status" "$response_time_ms"
  if [[ "$response_status" != "400" ]]; then
    echo "Authenticated invalid scan should return 400, got $response_status" >&2
    exit 1
  fi
  mark_check
}
...
PY
  ...
  python3 - <<'PY' "$metrics_json" "$base_url" "$checks_total" "$checks_passed" "$endpoint_json" "$status_json" "$avg_ms" "$p95_ms" "$status_total" "$success_2xx" "$auth_checks_skipped"
import json
import sys
...
auth_checks_skipped = sys.argv[11].lower() == 'true'
...
metrics = {
    ...
    "resilience": "Authenticated paths verified when credentials are available" if not auth_checks_skipped else "Authenticated paths skipped (no credentials)",
    "auth_checks_skipped": auth_checks_skipped,
}
...
if token="$(check_token)"; then
  check_authenticated_story_routes "$token"
fi
